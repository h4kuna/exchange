...
//-----------------download actual rating list----------------------------------
    ...
    protected function loadList() {
        ...
        $download = FALSE;
        if (!file_exists($new) || filesize($this->db->getConfig('database')) < 1) {//stahne rocni data
        ...
        if ($download) {
            ...
            if ($cnb !== FALSE && $cnb2 !== FALSE) {
                $this->createCache(Math::stroke2point($cnb . $cnb2));
            } else {
...
//-----------------methods whose create cache ----------------------------------
    ...
    protected function createCache($cnb) {
        if ($this->isDateLine($cnb)) {//uloží roční data
            $this->createYearCache($cnb);
            file_put_contents($this->file, '');
            $this->update = TRUE;
            $this->loadList();
        } else {//uloží denní data
...
//-----------------inherit methods----------------------------------------------
    ...
    public function loadCurrencies(array & $codes) {
        ...
        $result = $this->db->query(
            ...
            ORDER BY `' . self::C_DATE . '` DESC', \array_keys($code));
        foreach ($result as $v) {
    ...
    public function & getAllCode($codeAsId = FALSE) {
        $codes = $this->db->query(
                'SELECT %n, %n
            FROM %n
            ORDER BY %n ASC', self::C_CODE, self::C_ID_CURRENCY, self::T_CNB, self::C_CODE);
...
//-----------------sql-----------------
    ...
    private function insertHistory($czechDate) {
        ...
        try {
            $this->db->query('INSERT INTO %n (%n) VALUES (%s)', self::T_CNB_HISTORY, self::C_DATE, $czech);
            ...
            $idHis = $this->db->query('SELECT %n FROM %n WHERE %n=%s  LIMIT 1', self::C_ID_HISTORY, self::T_CNB_HISTORY, self::C_DATE, $czech)
                    ->fetchSingle();
        }
        return $idHis;
    }
    ...
    private function insertCurrency(array $insert) {
        try {
            ...
            return $this->db->query('SELECT %n FROM %n WHERE %and LIMIT 1', self::C_ID_CURRENCY, self::T_CNB, $insert)->fetchSingle();
        }
    }
    ...
    private function insertRating($code, $id, $idHis, $to) {
        if ($code !== self::CZK) {
            try {
                $this->db->query('INSERT INTO %n %v', self::T_CNB_RATE, array(self::C_OID_CURRENCY => $id,
    ...
    private function replaceCurrency($data) {
        ...
        $this->db->query('UPDATE %n SET %a WHERE %n=%i', self::T_CNB, $data, self::C_ID_CURRENCY, $id);
        return $id;
    }
